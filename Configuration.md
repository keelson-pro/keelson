...
|---|---|---|
| `KEELSON_SCOPE`<br>`Keelson/Scope` | `cluster` | `cluster` watches every namespace; `namespace` watches only the namespaces named in `KEELSON_NAMESPACES`. |
| `KEELSON_NAMESPACES`<br>`Keelson/Namespaces` | empty | Empty means every namespace, which is why the shipped default is empty and why `KEELSON_SCOPE=cluster` is happy with it; `KEELSON_SCOPE=namespace` requires at least one. A value is checked whichever scope is set, and a value under `cluster` scope is warned about at boot rather than silently ignored. One namespace or several: separate them with spaces or commas, whichever reads better where you set it (`"team-a team-b"` and `"team-a,team-b"` are the same list). Runs of whitespace collapse, empty entries from a doubled or trailing separator are dropped, and a namespace listed twice is watched once and warned about. Every entry must be a valid namespace name (lowercase letters, digits and hyphens, not leading or trailing, 63 characters at most) or boot fails naming the bad one. Each one is then read from the API server at boot, and anything short of a clean read fails validation, because a typo is otherwise invisible: listing a namespace that is not there returns an empty list rather than an error, so the watcher would stream nothing and report itself healthy forever. That read needs `get namespaces`, granted by the plain `keelson-clusterrole` and its binding: cluster-scoped objects that only a namespace-scoped install actually uses, since cluster scope has no list to check. Missing namespace, missing ClusterRole, unreachable API server: all three are broken installs, and Keelson refuses to boot into any of them rather than come up Ready and update nothing. Each namespace costs one `kubectl --watch` process per watched kind and one `kubectl get` per kind per reconcile, and each needs its own copy of the `-own-ns` Role and RoleBinding: without one, that namespace's watcher backs off on `Forbidden` alone while the rest keep streaming. |
...
| `KEELSON_RECONCILE_INTERVAL`<br>`Keelson/ReconcileInterval` | `600` (10m) | Seconds between reconcile scans: how often Keelson lists the cluster to refresh its workload cache and forget what has gone. A fallback and a safety net, not the thing that drives updates. Registry polling runs off each workload's own `next-due`, checked every `TickInterval`, so raising this costs cache freshness rather than update latency. Watch events carry additions and changes as they happen, and a watcher that dies replays the whole cluster when it respawns, so the one thing only this pass can see is a **deletion missed while a watcher was down**, since nothing arrives for an object that is gone. That is what sets the cadence, and what a stale record costs in the meantime is one workload polled that need not have been. A scan is one `kubectl` list per watched kind, so this is also the main lever on Keelson's load against the API server. Measured from the previous scan's start; long scans queue the next for the very next tick, never overlap, and warn via `reconcile-pass-overrun`. |
...
- **`keel.sh/pollSchedule` as a raw cron expression** — Keel accepts robfig
  ...
  Keelson reads every `@` descriptor, but warns with `poll-schedule-invalid`
  on a raw cron expression and falls back to
  `KEELSON_REGISTRY_POLL_INTERVAL_DEFAULT`. A cron expression addresses
  positions in a calendar rather than a cadence; express the cadence as a
  duration instead. Longer polling periods are at the mercy of node stability
